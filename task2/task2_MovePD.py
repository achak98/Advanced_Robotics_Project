...
print('Initial Position: {}'.format(sim.getJointPosition(endEffector).flatten()))
targetPosition = np.array([0.6, 0.23,   1.0045]) # y makes arm move left and right, x forward and back
print('Target Position: {}'.format(targetPosition))
...
print('Simulation Finished, end effecttor position: {}'.format(sim.getJointPosition(endEffector).flatten()))
print('Difference : {}'.format(targetPosition - sim.getJointPosition(endEffector).flatten()))
